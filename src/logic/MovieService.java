...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		mongo = MongoClients.create("mongodb://root:password@localhost:27017/?authSource=admin");
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		//TODO : implement
		FindIterable<Document>  result = movies.find(and(gte("votes", minVotes),gte("rating", minRating))).sort(Sorts.descending("votes", "rating")).limit(limit);
		return result;
	}
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		...
		//FindIterable<Document> movie = tweets.find(Filters.regex("text",pattern)).projection(new Document("movie", 1));
		//FindIterable<Document> result = movies.find(eq("title", movie)).limit(limit);

		Document query = new Document("tweets", new Document("$regex", keyword));
		FindIterable<Document> result = movies.find(query).limit(limit);

		// This doesn't work
		/*Document regQuery = new Document();
		regQuery.append("$regex", "*"+keyword+"*");
		regQuery.append("$option", "i");
		Document findQuery = new Document();
		findQuery.append("text", regQuery);
		FindIterable<Document> movie = tweets.find(findQuery).projection(new Document("movie", 1));
		FindIterable<Document> result = movies.find(eq("title", movie));
		result.limit(limit);
	...
	public FindIterable<Document>  getGeotaggedTweets(int limit) {
		//TODO : implement
		FindIterable<Document>  result = tweets.find(exists("geo.coordinates")).limit(limit);
		return result;
	}
